...
for i, metric in enumerate(METRIC_LIST):
    ...
    max_y = 0
    min_y = 0
    for alpha, c, m in zip(ALPHA_LIST, colors, markers):
        # Ignore baseline
        if alpha =='0.5':
            continue

        y = getattr(results[alpha], metric)
        compressed_y = [i for i in y if i is not None]
        if len(compressed_y) == 0:
            continue

        max_y = max(max_y, max(compressed_y))
        min_y = min(min_y, min(compressed_y))
        axs_m[metric].scatter(x, y, label='\u03B1={}'.format(alpha), color=c, marker=m, s=marker_size)
    ...
    axs_m[metric].axhline(-0.01, 0, 50, color='k')
...
figs.savefig('alphas.png', dpi=300)
plt.show()
