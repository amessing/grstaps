...
namespace grstaps::icra2021
    {
        std::unique_ptr<OldIcraProblemV1> OldIcraProblemV1::generate(nlohmann::json& config)
        {
            ...
            std::unique_ptr<OldIcraProblemV1> problem = std::make_unique<OldIcraProblemV1>();
            ...
            // Carry Medicine to Hospital
            {
                ...
                std::vector<float> traits = {0, 0, 0, 0, 0, 0};
                const std::vector<float> noncumulative_traits = {0, 0, 0, 0, 0, 0};
                for(unsigned int i = 0; i < num_survivors; ++i)
                {
