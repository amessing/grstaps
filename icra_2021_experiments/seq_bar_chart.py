...
class Display:
    ...
    def run(self):
        ...
        labels = ['Makespan', 'Comp. Time', 'Nodes\nExpanded', 'Nodes\nVisited']
        ...
        self.axs.bar(x, y, tick_label=labels, color='r')
        self.axs.axhline(0.0, 0, len(METRIC_LIST), color='k')
        self.axs.set_title("Sequential")
        self.axs.set_ylim((min_y * 1.1, max_y * 1.1))
        self.axs.set_ylabel("% Difference")
        self.axs.set_yscale('symlog')
        self.axs.set_xticklabels(labels, rotation=90, ha='center')
        self.setSize()

    def setSize(self, width=0.5, height=0.375):
    ...
    def save(self):
        self.figs.savefig('sequential.png', dpi=300)
