...
// Computes the order relationships among time points
    void Linearizer::computeOrderMatrix()
    {
        ...
        matrix[lastPoint -
               1][lastPoint] = iteration;                            // The same for the new step to be added
        for(unsigned int i = 0; i < basePlanComponents.size(); i++)
        {
        ...
        *repeat = false;
        ...
        while(pq.size() > 0)
        {
            ...
            {    // Start time point of an action
                ...
                {
                    //if (debug)	cout << "Delay: action " << tp << ": " << time[start] << " = " << time[start + 1] << " - " << dur << endl;
            ...
            {    // Wrong order, p should be before linearOrder[prev]
                ...
                do
                {
                    ...
                    {    // Invalid plan: doesn\92t meet temporal contraints
                        //if (this->plan == nullptr || this->plan->id == PLAN_DEB) cout << "Invalid plan: doesn\92t meet temporal contraints" << endl;
                        return true;
                    }
